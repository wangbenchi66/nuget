...
线上nuget引入 版本号随时更新
``` xml
<PackageReference Include="Easy.SqlSugar.Core" Version="2026.05.06.1" />
```
### 3.1.1 SqlSugar配置文件
``` json
...
//ISqlSugarClient获取对应的示例、仓储
//例如 是注入获取的
 private readonly ISqlSugarClient _journalClient;
 private readonly ISqlSugarClient _userClient;
 private readonly SimpleClient<User> _userSimpleClient;
    private readonly BaseSqlSugarRepository<User> _userReps;


    public SqlSugarDiApis(ISqlSugarClient sqlSugarClient)
    {
        _journalClient = sqlSugarClient.GetConfigDb("journal");//获取指定数据库
        _userClient = sqlSugarClient.GetTenantDb<User>();//根据实体特性获取数据库
        _userSimpleClient = sqlSugarClient.GetConfigDbRepository<User>();//获取指定表的仓储
        _userReps = _sqlSugarClient.GetBaseRepository<User>();//获取指定表的仓储
    }
```
### 雪花id配置
``` csharp
